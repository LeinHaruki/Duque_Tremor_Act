                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge text-white
                                        @if($patient->gender == 'Male') bg-primary
                                        @elseif($patient->gender == 'Female') bg-danger
                                        @else bg-secondary
                                        @endif">
                                        <i class="fas 
                                            @if($patient->gender == 'Male') fa-mars
                                            @elseif($patient->gender == 'Female') fa-venus
                                            @else fa-genderless
                                            @endif"></i>
                                        {{ $patient->gender }}
                                    </span>
                                ...
                                <td>
                                    <i class="fas fa-phone text-success"></i> {{ $patient->contact }}
                                </td>
                                <td>
                                    <small class="text-muted">{{ Str::limit($patient->address, 30) }}</small>
                                </td>
                                <td>
                                    <span class="badge bg-info">{{ $patient->appointments->count() }} appointments</span>
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <form action="{{ route('patients.destroy', $patient) }}" 
                                              method="POST" 
                                              class="d-inline"
                                              onsubmit="return confirmDelete('{{ $patient->full_name }}')">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
